...
export default function InventoryPage() {
  ...
  const stats = useMemo(() => ({
    ...
    outOfStock: materials.filter(m => m.Status === "Out of Stock" || m.Status === "Depleted").length,
    totalFt: materials.reduce((acc, m) => acc + parseNum(m["Total Remaining (ft)"]), 0),
  }), [materials]);

  if (loading) {
    return (
      <div className="p-8 flex items-center justify-center min-h-screen bg-slate-50/80 dark:bg-[#2e388e0d]">
  ...
  return (
    <div className="p-4 md:p-8 bg-slate-50/80 dark:bg-[#2e388e0d] min-h-screen pb-32">
      {/* Header */}
