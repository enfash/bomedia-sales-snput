...
export async function GET() {
  try {
    ...
    if (!mSheet) {
      ...
      mSheet = await doc.addSheet({
        title: MATERIALS_SHEET,
        headerValues: MATERIALS_HEADERS,
      });
    } else {
    ...
    for (const mId in groups) {
      ...
      const totalRemaining = groupRolls.reduce(
        (sum, r) => sum + (parseFloat(r.get("Remaining Length (ft)")) || 0),
        0,
      );
      const totalCapacity = groupRolls.reduce(
        (sum, r) => sum + (parseFloat(r.get("Total Length (ft)")) || 0),
        0,
      );
      ...
      const activeRoll =
        groupRolls
          ...
          .sort((a, b) =>
            (a.get("Roll ID") || "").localeCompare(b.get("Roll ID") || ""),
          )[0] ||
        groupRolls.sort((a, b) =>
          (a.get("Roll ID") || "").localeCompare(b.get("Roll ID") || ""),
        )[0];
      ...
      const threshold =
        parseFloat(firstRoll.get("Low Stock Threshold (ft)") || "20") || 20;
      ...
      const mRow = materialRows.find((r) => r.get("Material ID") === mId);
      if (mRow) {
